...
- **Deterministic source monitoring for trade alerts** — Kodiak Server can now monitor configured external feeds, classify explicit `buy` / `sell` posts with regex rules, persist recent alerts plus per-source runtime state, and expose them through new REST endpoints at `GET /api/v1/signals/overview`, `GET /api/v1/signals/alerts`, `GET /api/v1/signals/sources`, and `POST /api/v1/signals/poll`. Unmatched items are preserved in a `needs_inference` bucket instead of forcing an LLM decision. X monitoring now supports first-party OAuth 2.0 user-context tokens handed off from BearClawWeb and refreshed directly by Kodiak, with encrypted token storage plus new auth endpoints at `GET /api/v1/signals/x/oauth/status`, `POST /api/v1/signals/x/oauth/connect`, and `DELETE /api/v1/signals/x/oauth`.
